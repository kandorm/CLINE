...
from lecbert import LecbertConfig
...
# Copied from transformers.modeling_roberta.RobertaEmbeddings
class LecbertEmbeddings(nn.Module):
    """
...
# Copied from transformers.modeling_bert.BertSelfAttention with Bert->Lecbert
class LecbertSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class LecbertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.modeling_bert.BertAttention with Bert->Lecbert
class LecbertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = LecbertSelfAttention(config)
        self.output = LecbertSelfOutput(config)
...
class LecbertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.modeling_bert.BertLayer with Bert->Lecbert
class LecbertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = LecbertAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = LecbertAttention(config)
        self.intermediate = LecbertIntermediate(config)
        self.output = LecbertOutput(config)
...
# Copied from transformers.modeling_bert.BertEncoder with Bert->Lecbert
class LecbertEncoder(nn.Module):
    """
    Same as RobertaEncoder.
    """
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([LecbertLayer(config) for _ in range(config.num_hidden_layers)])
...
# Copied from transformers.modeling_roberta.RobertaPreTrainedModel with Roberta->Lecbert
class LecbertPreTrainedModel(PreTrainedModel):
    """An abstract class to handle weights initialization and
    a simple interface for downloading and loading pretrained models.
    """

    config_class = LecbertConfig
    base_model_prefix = "lecbert"
...
# Copied from transformers.modeling_bert.BertModel with Bert->Lecbert
class LecbertModel(LecbertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = LecbertEmbeddings(config)
        self.encoder = LecbertEncoder(config)

        self.pooler = LecbertPooler(config) if add_pooling_layer else None
...
# Copied from transformers.modeling_roberta.RobertaForMaskedLM with Roberta->Lecbert
class LecbertForMaskedLM(LecbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `LecbertForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.lecbert = LecbertModel(config, add_pooling_layer=False)
...
# Copied from transformers.modeling_roberta.RobertaForSequenceClassification with roberta -> lecbert
class LecbertForSequenceClassification(LecbertPreTrainedModel):
    authorized_missing_keys = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.lecbert = LecbertModel(config, add_pooling_layer=False)
        self.classifier = LecbertClassificationHead(config)
...
# Copied from transformers.modeling_roberta.RobertaForMultipleChoice with roberta -> lecbert
class LecbertForMultipleChoice(LecbertPreTrainedModel):
    authorized_missing_keys = [r"position_ids"]
...
# Copied from transformers.modeling_roberta.RobertaForTokenClassification with roberta -> lecbert
class LecbertForTokenClassification(LecbertPreTrainedModel):
    authorized_unexpected_keys = [r"pooler"]
    authorized_missing_keys = [r"position_ids"]
...
# Copied from transformers.modeling_roberta.RobertaClassificationHead with roberta -> lecbert
class LecbertClassificationHead(nn.Module):
...
# Copied from transformers.modeling_roberta.RobertaForQuestionAnswering with roberta -> lecbert
class LecbertForQuestionAnswering(LecbertPreTrainedModel):
    authorized_unexpected_keys = [r"pooler"]
    authorized_missing_keys = [r"position_ids"]
